import os

from flask import Flask, jsonify, render_template, request

from rag.pipeline import RAGPipeline

app = Flask(__name__)

BOOKS_PATH = "data/books.csv"
FAQ_PATH = "data/faq.json"
CHAT_TOP_K = 6

pipeline = None
pipeline_mtimes = {}


def _data_file_mtimes():
    """Lay thoi diem sua file data de reload pipeline khi CSV/FAQ thay doi."""
    return {
        BOOKS_PATH: os.path.getmtime(BOOKS_PATH),
        FAQ_PATH: os.path.getmtime(FAQ_PATH),
    }


def get_pipeline():
    """Khoi tao pipeline mot lan, sau do tai lai neu data thay doi."""
    global pipeline, pipeline_mtimes

    current_mtimes = _data_file_mtimes()
    if pipeline is None or current_mtimes != pipeline_mtimes:
        pipeline = RAGPipeline(BOOKS_PATH, FAQ_PATH)
        pipeline_mtimes = current_mtimes

    return pipeline


@app.route("/")
def home():
    return render_template("index.html")


@app.route("/chat", methods=["POST"])
def chat():
    data = request.get_json(silent=True) or {}
    query = data.get("message", "").strip()

    if not query:
        return jsonify({"response": "Vui lòng nhập câu hỏi."})

    response, retrieved_docs = get_pipeline().answer(query, top_k=CHAT_TOP_K)

    return jsonify({
        "response": response,
        "retrieved_docs": retrieved_docs,
    })


if __name__ == "__main__":
    app.run(debug=True)
